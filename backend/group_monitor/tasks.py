...
def group_monitor_task(self, job_id: int):
    ...
    with get_short_session() as db:
        ...
        if not is_safe:
            logger.warning(f"group_monitor_task: Safety Circuit Breaker triggered for account {account_id}: {reason}")
            job.status = "failed"
    ...
    if not lock.acquire(blocking=False):
        logger.warning(
            f"group_monitor_task: account {account_id} is locked. Job {job_id} will retry."
        )
